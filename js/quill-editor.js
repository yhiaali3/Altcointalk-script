...
`;
      ...
      if (quillImageButton) {
        ...
        newBtn.addEventListener('click', function (e) {
          ...
          input.onchange = async function (ev) {
            ...
            const apiKey = 'chv_UuLJ_cf38bcec45d9f7f5e38a65bb4a034e2b36bf8254618d85adcc9e52434e45a2457e709d8a601acbe958b8a1908b7d8c40cfc524fd1d6859a0415a87a71cfa802e'; // You may set an API key if requested in the future
  ...
  function initQuillEditor() {
    try {
      ...
      try {
        quill.root.addEventListener('keydown', function (e) {
          try {
            ...
            // ✅ Ctrl+Shift (any) → simulates pressing the toolbar direction toggle
            if (e.ctrlKey && e.shiftKey && !(key === 'r' || key === 'R' || key === 'l' || key === 'L')) {
              e.preventDefault();
              e.stopPropagation();
              try {
                // Find the direction toggle button (T← or T→)
      ...
      try {
        ...
        // 💡 Modify alignment buttons to insert BBCode directly
        try {
          ...
          alignBtns.forEach(function (btn) {
            try {
              ...
              btn.addEventListener('click', function (e) {
                ...
                // Choose BBCode tags based on the button value
                var openTag = '';
                var closeTag = '';
        ...
        if (codeBtn) {
          codeBtn.addEventListener('click', function (e) {
            ...
            if (range.length > 0) {
              ...
              // Delete the selected text
              quill.deleteText(range.index, range.length);
              // Insert the selected text inside the code template
              insertText = '[code]\n' + selectedText + '\n[/code]';
      ...
      function normalizeBlankLines(s) {
        ...
        // Collapse any 3+ empty lines into two
        out = out.replace(/\n{3,}/g, '\n\n');

        // Remove spacing between adjacent BBCode tags
        out = out.replace(/\]\s*\n+\s*(?=\[)/g, ']\n');
        ...
        function walk(node) {
          ...
          node.childNodes && Array.from(node.childNodes).forEach(function (ch) {
            if (ch.nodeType === 3) {
              ...
              // --- Add support for <sup>, <sub>, <tt>, <marquee>, <details> ---
              if (tag === 'sup') {
                t += '[sup]' + walk(ch) + '[/sup]';
              } else if (tag === 'sub') {
                t += '[sub]' + walk(ch) + '[/sub]';
      ...
      (function () {
        ...
        try {
          ...
          var possibleKeywords = ['preview', 'post', 'submit', 'publish', 'save', 'send'];
  ...
  (function quillToggleUI() {
    ...
    btnEnable.addEventListener('click', function (event) {
      ...
      try {
        // keep legacy single-key flag for other scripts
        try { chrome.storage.local.set({ quillEnabled: true }); } catch (e) { }
        // also update the structured settings so emoji-toolbar listens correctly
        try {
          chrome.storage.local.get('altcoinstalks', function (res) {
            ...
            try { chrome.storage.local.set({ altcoinstalks: s }); } catch (e) { }
          });
        } catch (e) { }
      } catch (e) { }
      ...
      // Hide any visible external emoji toolbars immediately (responsive UX)
    ...
    btnDisable.addEventListener('click', function (event) {
      ...
      // Ensure any removed/hidden emoji toolbars are reattached by the toolbar script
