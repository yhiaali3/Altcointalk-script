(function () {
    ...
    function dispatchQuoteOnce(text) {
        try {
            if (!window.__alt_dispatchedQuotes) window.__alt_dispatchedQuotes = new Map();
            var map = window.__alt_dispatchedQuotes;
            var now = Date.now();
            ...
            if (map.has(key)) { console.log('[ALT-INJECT] quote dispatch suppressed (duplicate)'); return false; }
            map.set(key, now);
            window.dispatchEvent(new CustomEvent('alt-quote-text', { detail: { text: text } }));
            return true;
        } catch (e) {
            try { window.dispatchEvent(new CustomEvent('alt-quote-text', { detail: { text: text } })); } catch (e2) { }
            return true;
        }
    }
    ...
    async function tryFetchHtml(u) {
        try {
            console.log('[ALT-INJECT] tryFetchHtml start', u);
            var uu = u;
            if (!/^https?:\/\//i.test(uu)) {
                if (uu.charAt(0) === '/') uu = location.origin + uu;
                else uu = location.origin + '/' + uu;
            }
            console.log('[ALT-INJECT] tryFetchHtml absolute', uu);
            var r = await fetch(uu, { credentials: 'same-origin' });
            if (!r.ok) { console.log('[ALT-INJECT] tryFetchHtml fetch failed', r.status); return null; }
            var h = await r.text();
            var has = /name=["']?message["']?/i.test(h);
            console.log('[ALT-INJECT] tryFetchHtml fetched, contains message textarea:', has);
            if (has) return h;
            return null;
        } catch (e) { console.log('[ALT-INJECT] tryFetchHtml error', e); return null; }
    }
    ...
    function withTemporaryInterceptors(timeoutMs, onResponse) {
        ...
        function patchedXHROpen() {
            try {
                ...
                this._alt_original_url = arguments[1];
            } catch (e) { }
    ...
    document.addEventListener('click', function (ev) {
        try {
            ...
            (function () {
                ...
                try {
                    console.log('[ALT-INJECT] calling page insertQuote if available');
                    ...
                    if (typeof origGetXMLDocument === 'function') {
                        patchedGetXMLDocument = function (url, callback) {
                            try {
                                console.log('[ALT-INJECT] patched getXMLDocument called for', url);
                                var wrappedCallback = function (doc) {
                                    try {
                                        ...
                                        if (txt) {
                                            try { console.log('[ALT-INJECT] getXMLDocument response preview', txt.slice(0, 300)); } catch (e) { }
                                        }
                                        if (txt) {
                        ...
                        try { window.getXMLDocument = patchedGetXMLDocument; console.log('[ALT-INJECT] getXMLDocument monkey-patched'); } catch (e) { }
                    } else {
                        console.log('[ALT-INJECT] getXMLDocument not found');
                    }
                    ...
                    var wrapped = function (doc) {
                        try {
                            ...
                            if (txt) {
                                try { console.log('[ALT-INJECT] onDocReceived response preview', txt.slice(0, 300)); } catch (e) { }
                            }
                            if (txt) {
                                try {
                                    var quoted = '';
                    ...
                    setTimeout(function () {
                        try {
                            if (patchedGetXMLDocument && window.getXMLDocument === patchedGetXMLDocument) {
                                ...
                                console.log('[ALT-INJECT] restored original getXMLDocument');
                            }
                        } catch (e) { }
                    }, 2000);
                } catch (e) { console.log('[ALT-INJECT] error invoking insertQuote', e); cancel(); }
