(function () {
  try {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (/\baltcoinstalks\.com\b/.test(url)) {
        // load scripts only when on altcoinstalks.com
        if (!window.jQuery) {
        ...
        if (root) {
          root.innerHTML = '<div style="font-family: Arial, Helvetica, sans-serif; padding:24px; box-sizing:border-box; width:100%; height:100%; display:flex; align-items:center; justify-content:center;">'
            ...
            + '<h2 style="margin:0 0 8px 0; font-size:18px;">This extension is exclusively for Altcointalks.</h2>'
            + '<p style="margin:0; color:#444;">Open Altcointalks.org to view the extension tools.</p>'
            + '</div></div>';
        } else {
          document.body.innerText = 'This extension is exclusively for Altcointalks.';
        }
      }
    });
  } catch (e) {
