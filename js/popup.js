...
$(function () {
  ...
  // load settings
  chrome.storage.local.get('altcoinstalks', (result) => {
    const s = result && result.altcoinstalks ? result.altcoinstalks : {};
    // zoom
    let zoom = 100;
    ...
    document.getElementById("resetTheme").addEventListener("click", () => {
      ...
      // remove theme key inside storage object
      chrome.storage.local.get('altcoinstalks', (res) => {
        const b = (res && res.altcoinstalks) ? res.altcoinstalks : {};
        if (b.hasOwnProperty('theme')) delete b.theme;
        chrome.storage.local.set({ altcoinstalks: b }, () => {
    ...
    // Altcoin selection controls: checkboxes in popup to choose extra coins to display
    try {
      const checkboxes = Array.from(document.querySelectorAll('.altcoin-checkbox'));
      const defaultCoins = ['bitcoin', 'ethereum'];
      chrome.storage.local.get('altcoinstalks', (res) => {
        const existing = res && res.altcoinstalks ? res.altcoinstalks : {};
        const selected = Array.isArray(existing.altcoins) && existing.altcoins.length > 0 ? existing.altcoins : defaultCoins;
        checkboxes.forEach(cb => {
          try {
            const id = cb.getAttribute('data-coin-id');
      ...
      checkboxes.forEach(cb => cb.addEventListener('change', () => {
        try {
          const selected = Array.from(document.querySelectorAll('.altcoin-checkbox:checked')).map(x => x.getAttribute('data-coin-id'));
          // ensure at least bitcoin and ethereum by default if user unchecks all, keep empty allowed
          saveKeyValue('altcoins', selected);
          // ask content script to refresh prices immediately
          chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            for (const tab of tabs) {
              try { chrome.tabs.sendMessage(tab.id, { type: 'update-prices' }); } catch (e) { }
            }
          });
        } catch (e) { }
      }));
  ...
  // helper: save altcoinstalks object partially
  function saveKeyValue(key, value) {
    chrome.storage.local.get('altcoinstalks', (res) => {
      const existing = res && res.altcoinstalks ? res.altcoinstalks : {};
      existing[key] = value;
      chrome.storage.local.set({ altcoinstalks: existing }, () => {
  ...
  // toggles
  // emoticon replacer controls removed
          ...
          for (const tab of tabs) {
            try {
              chrome.tabs.sendMessage(tab.id, {
                ...
                cssPath: 'css/altcoinstalks/colorblind-safe.css'
              });
            } catch (e) { }
          }
        });
  ...
  exportBtn.addEventListener('click', () => {
    chrome.storage.local.get('themes', (data) => {
      ...
      a.href = url;
      a.download = 'altcoinstalks-themes.json';
      a.click();
